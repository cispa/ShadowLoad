...
// gadget used for prefetchjection_offset
...
static long device_ioctl(struct file *file, unsigned int ioctl_num,
  ...
  case CMD_GADGET: {
      i = (uintptr_t)kernel_buffer + secret[ioctl_param];
      asm volatile("mfence");
...
int init_module(void) {
  ...
  if (r != 0) {
    printk(KERN_ALERT "[fetch-probe-intel] Failed registering device with %d\n", r);
    return 1;
  }
  ...
  printk(KERN_INFO "[fetch-probe-intel] buffer address: 0x%016llx\n", (uint64_t)kernel_buffer);
  printk(KERN_INFO "[fetch-probe-intel] spectre gadget address: 0x%016llx\n", (uint64_t)__gadget);
...
void cleanup_module(void) {
  ...
  printk(KERN_INFO "[fetch-probe-intel] Removed.\n");
}
