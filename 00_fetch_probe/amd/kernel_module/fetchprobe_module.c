...
MODULE_DESCRIPTION("FetchProbe PoC Kernel Module (AMD)");
MODULE_LICENSE("GPL");
...
static long device_ioctl(struct file *file, unsigned int ioctl_num,
  ...
  case CMD_GADGET_OFF: {
      // *(kernel_buffer + secret[offset] * 4)
      asm volatile(".global __gadget_off\n__gadget_off:\n mov (%0), %%al" :: "r" (&kernel_buffer[((secret[ioctl_param / 8] & (1 << (ioctl_param % 8))) != 0) * 4]) : "rax");
      break;
  }
...
int init_module(void) {
  ...
  if (r != 0) {
    printk(KERN_ALERT "[fetchprobe-poc-amd] Failed registering device with %d\n", r);
    return 1;
  }
  ...
  printk(KERN_INFO "[fetchprobe-poc-amd] buffer address: 0x%016llx\n", (uint64_t)kernel_buffer);
  printk(KERN_INFO "[fetchprobe-poc-amd] control-flow gadget address: 0x%016llx\n", (uint64_t)__gadget_cf);
  printk(KERN_INFO "[fetchprobe-poc-amd] offset gadget address: 0x%016llx\n", (uint64_t)__gadget_off);
...
void cleanup_module(void) {
  ...
  printk(KERN_INFO "[fetchprobe-poc-amd] Removed.\n");
}
